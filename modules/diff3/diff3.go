...
func SimpleMerge(ctx context.Context, textO, textA, textB string, labelO, labelA, labelB string) (string, bool, error) {
	o := strings.Split(textO, "\n")
	a := strings.Split(textA, "\n")
	b := strings.Split(textB, "\n")
	regions := Diff3Merge(a, o, b, true)
	var conflicts = false
	var lines []string
