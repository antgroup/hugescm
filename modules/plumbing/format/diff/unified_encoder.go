...
func (e *UnifiedEncoder) writeFilePatchHeader(sb *strings.Builder, filePatch FilePatch) {
	...
	isFragments := filePatch.IsFragments()
	...
	case from != nil && to != nil:
		...
		if !hashEquals {
			lines = e.appendPathLines(lines, e.srcPrefix+from.Path(), e.dstPrefix+to.Path(), isBinary, isFragments)
		}
	case from == nil:
		lines = append(lines,
		...
		lines = e.appendPathLines(lines, "/dev/null", e.dstPrefix+to.Path(), isBinary, isFragments)
	case to == nil:
		lines = append(lines,
		...
		lines = e.appendPathLines(lines, e.srcPrefix+from.Path(), "/dev/null", isBinary, isFragments)
	}
