...
func TestViewport_SelectionOff_WrapOff_SetHighlightsStyledContent(t *testing.T) {
	...
	expectedView := Pad(vp.GetWidth(), vp.GetHeight(), []string{
		"header",
		RedFg.Render("the first line"),
		GreenFg.Render("the ") + BlueFg.Render("second") + GreenFg.Render(" line"),
		BlueFg.Render("the ") + RedFg.Render("third") + BlueFg.Render(" line"),
		"75% (3/4)",
	})
	CmpStr(t, expectedView, vp.View())
}
