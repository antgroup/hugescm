...
func TestMerge0(t *testing.T) {
	...
	defer odb.Close()
	o, err := odb.Tree(context.TODO(), plumbing.NewHash("dcfe2d5aaa20344a565da7724516700a761c7695285b47ed2e097e44eb6c7b55"))
	if err != nil {
		fmt.Fprintf(os.Stderr, "new o %v\n", err)
		return
	}
	a, err := odb.Tree(context.TODO(), plumbing.NewHash("9c3c905c4d8b1c6c4990beb3a56184a2b325a780d9e199a08cb8aa8440822dee"))
	if err != nil {
		fmt.Fprintf(os.Stderr, "new a %v\n", err)
		return
	}
	b, err := odb.Tree(context.TODO(), plumbing.NewHash("d736c423e7a4d726f6fefe0e382d4cfd4f16b31f3b2aaca2732a87437c9d65bf"))
	if err != nil {
		fmt.Fprintf(os.Stderr, "new b %v\n", err)
		return
	}
	d, err := odb.mergeDifferences(context.Background(), o, a, b)
	if err != nil {
	...
	result, err := odb.MergeTree(context.TODO(), o, a, b, &MergeOptions{Textconv: true})
	if err != nil {
...
func TestMerge1(t *testing.T) {
	...
	a, err := odb.Tree(context.TODO(), plumbing.NewHash("117c2199f51dd4e9bda78cab847d59f58fb46f7adc7c3b52dbe95b2916747814"))
	if err != nil {
		fmt.Fprintf(os.Stderr, "new a %v\n", err)
		return
	}
	b, err := odb.Tree(context.TODO(), plumbing.NewHash("399ad3c84a2d386b3bb58c6875f4b9358eda3809ac5a4c477a7eeab010d7ff38"))
	if err != nil {
		fmt.Fprintf(os.Stderr, "new b %v\n", err)
		return
	}
	...
	result, err := odb.MergeTree(context.Background(), o, a, b, &MergeOptions{Textconv: true, Branch1: "dev-2"})
	if err != nil {
		fmt.Fprintf(os.Stderr, "merge tree: %v\n", err)
		return
	}
...
`
	...
	s, conflict, err := d.ExternalMerge(context.Background(), textO, textA, textB, "a.txt", "a.txt", "b.txt")
	if err != nil {
		return
	}
	fmt.Fprintf(os.Stderr, "conflict: %v\n\n\n%s\n", conflict, s)
...
`
	...
	s, conflict, err := d.Diff3Merge(context.Background(), textO, textA, textB, "a.txt", "a.txt", "b.txt")
	if err != nil {
		return
	}
	fmt.Fprintf(os.Stderr, "conflict: %v\n\n\n%s\n", conflict, s)
