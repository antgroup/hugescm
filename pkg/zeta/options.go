...
func (opts *DiffContextOptions) PatchContext(ctx context.Context, cs object.Changes) (*object.Patch, error) {
	return cs.PatchContext(ctx, opts.Textconv)
}

func (opts *DiffContextOptions) formatChanges(changes merkletrie.Changes, w io.Writer) error {
...
func (opts *DiffContextOptions) formatStats(w io.Writer, stats object.FileStats) error {
...
func IsErrUnexpectedEOF(err error) bool {
	return errors.Is(err, syscall.EPIPE) || err == io.ErrClosedPipe || err == io.ErrUnexpectedEOF
}

func (opts *DiffContextOptions) format(patch *object.Patch, w io.Writer) error {
	if opts.ShortStat {
		stats := patch.Stats()
		var added, deleted int
		for _, s := range stats {
		...
		fmt.Fprintf(w, " %d files changed, %d insertions(+), %d deletions(-)%c", len(stats), added, deleted, opts.NewLine)
		return nil
	}
	if opts.NumStat {
		stats := patch.Stats()
	...
	if opts.Stat {
		stats := patch.Stats()
		return opts.formatStats(w, stats)
	}
	if err := patch.EncodeEx(w, opts.UseColor); err != nil && !IsErrUnexpectedEOF(err) {
		return err
	}
	return nil
}

func nameFromFilePacth(p fdiff.FilePatch) (name string) {
	a, b := p.Files()
	if a != nil {
		name = a.Path()
	}
...
func statFormFilePatch(p fdiff.FilePatch) byte {
	a, b := p.Files()
	if a == nil {
		return 'A'
	}
...
func (opts *DiffContextOptions) formatEx(patch *object.Patch, w io.Writer) error {
	m := NewMatcher(opts.PathSpec)
	if opts.NameOnly {
		for _, p := range patch.FilePatches() {
			name := nameFromFilePacth(p)
			if !m.Match(name) {
				continue
			}
			fmt.Fprintf(w, "%s%c", name, opts.NewLine)
	...
	if opts.NameStatus {
		for _, p := range patch.FilePatches() {
			...
			fmt.Fprintf(w, "%c    %s%c", statFormFilePatch(p), name, opts.NewLine)
		}
		return nil
	}
	var patchs []fdiff.FilePatch
	for _, p := range patch.FilePatches() {
		name := nameFromFilePacth(p)
		if !m.Match(name) {
			continue
		}
		patchs = append(patchs, p)
	}
	return opts.format(object.NewPatch(patch.Message(), patchs), w)
}
